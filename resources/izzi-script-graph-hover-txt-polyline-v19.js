...
// Version 19 - Fixed Polyline Buffer with Working Debug
(function() {
    // ==================== CONFIGURATION ====================
    ...
    function init() {
        ...
        if (!svgElement) {
            log('SVG element not found, retrying...');
            setTimeout(init, 500);
            return;
        }
        ...
        // Create debug visuals if enabled (with a slight delay to ensure DOM is ready)
        if (debug_visuals) {
            setTimeout(createDebugVisuals, 100);
        }
    ...
    function extractPolylineSegments() {
        ...
        lineGraphs.forEach((lineGraph, index) => {
            ...
            log(`Graph ${lineGraph.id} has ${lineElements.length} line elements`);
            ...
            if (textEl) {
                ...
                if (!isNaN(x) && !isNaN(y)) {
                    ...
                    try {
                        ...
                        textBounds.set(lineGraph, { x: bbox.x + bbox.width/2, y: bbox.y + bbox.height/2 });
                        log(`Text for ${lineGraph.id} using bbox:`, bbox.x, bbox.y);
                    } catch(e) {
                        log('Could not get text bounds for', lineGraph.id);
                    }
                }
            }
        });
    ...
    function createDebugVisuals() {
        ...
        if (!svgElement) {
            log('SVG element not available for debug visuals');
            return;
        }
        
        // Create a debug group to hold all visuals
        let debugGroup = svgElement.getElementById('debug-visuals-group');
        if (!debugGroup) {
            debugGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
            debugGroup.setAttribute('id', 'debug-visuals-group');
        ...
        // Create circles at polyline segment points for debugging
        polylineSegments.forEach((segments, lineGraph) => {
            segments.forEach((seg, idx) => {
                // Create a circle at the midpoint of each segment
                ...
                // Also draw the actual line segment in light gray for reference
            ...
            if (textPos) {
                ...
                textCircle.setAttribute('fill-opacity', '0.2');
                textCircle.setAttribute('stroke', 'blue');
                textCircle.setAttribute('stroke-width', '1');
                textCircle.style.pointerEvents = 'none';
                debugGroup.appendChild(textCircle);
            }
        });
        
        log(`Created debug visuals with ${debugGroup.children.length} elements`);
    }
